...
One of the primary objectives is for you to learn the fundamentals of Computer Science.

In the first workshop, we used HTML by itself.

In the second workshop we used JavaScript by itself.

In this workshop, we are going to interact with HTML with JavaScript.

## The differences between HTML and JavaScript

In the first workshop, you added image, header, and link elements to an HTML page.

JavaScript can interact with those HTML elemetns by moving those elements around, making more elements, and remove elements entirely.
...
Functions are verbs. They usually make something happen, perform some kind of action, or change something in some way. You can think of functions as verbs. For example eat, move, turn on could all be functions.
...
In the case that a function has an input and output, you can say if you give the function an input, it will give you back some output.

#### Math analogy

If you ***were*** the `double` function (yes you the person reading this tutorial):
...
If you said "accio car keys", your car keys would magically fly towards you. Pretty useful if you've lost your car keys and had no idea where they were.
...
- and you would give me the **output** of my actual physical socks that were missing
...
- and you would give me back the **output** of the actual physical pot that was on the top shelf
...
// I can describe this by sayning "I am calling the double function with the input of 3"
```
...
- you would then perform the __action__ of making a popup box appear with the question `"Hello, what is your name?"` and a blank input field for the user to type in.
...
In the line of code above, I have no way of referring to that sock that it gave to me *unless* I have a **reference** to it through a variable. To reference the sock the `accio` function outputs, I can do something like this:

```js
var sock = accio("my missing sock")
```

Now in the rest of the program, whenever I use the variable `sock`, the program knows what I am **referrencing**. Another way I could say this is that the `sock` variable **refers** to my actual physical sock. Another way I could say it is that the `sock` variable **points** to my actual physical sock.
...
because I didn't **assign** a variable to the output of this function, I have no way of being able to **refer** to the output of this function ever again.

If the user types in `"Jonathan"`, I want to be able to reference that in the program again so I assign the output of the `prompt` function to a variable:
...
In programming, one way we try to model the world is through what we call an **object**.
...
One of my properties is my `name`. The value of that property is the string `"Jonathan"`

Another one of my properties is my `age` The value of my `age` property is the number `24`.

To make an empty object in JavaScript I can write `{}`
and so that I can refer to it later, I have to have a variable **point** to it:
...
The actual challenge:
    ...
    With your pair, think of another object (besides a person) and discuss what it's properties might be and values they might have.

    Another examples of another object is a car. It's properties could be make, model, year, gas mileage, etc.
```
...
```js
person1.walk // this would give me back the function with all the code I  wrote to walk:
...
person1.eat // this would give me back the function with all the code I  wrote to eat:
...
```js
person1.walk(); // this will run each line of code in the person1.walk function
                // one line at a time
...
The actual challenge:
    ...
    With your pair, think of an object (feel free to use the objects that you came up from your first challenge) and discuss what they might do.

    For a car car. It's properties that are functions could be drive, refill gas, turn on wind shield wiper
```

## The Coffee Shop Analogy

Say you're Alice, the creator of a take out coffee shop. Your coffee shop is run inside of a very high tech room that can:

1. Create any object you want
2. Find any object that is inside of the room given the identifier of the object

(*Don't read too much into the analogy, it's not perfect.*)

### How the coffee shop works:

Right now you need to program the part of the room that takes care of the take-out orders.

Here are the steps that you need:


```
- When the operating system that the room is running  is ready,
...
```js
room // is a defined variable that refers to thee magical room
...
Also, whenever the take out phone rings, the room will **automatically** call the phone's `onring` function.
...
So because whenever the phone rings, the room will **AUTOMATICALLY** run the `phone.onring` function. This means that you never have to call the `phone.onring()` function yourself, the room **AUTOMATICALLY** calls it!

Moreover, when the operating system of the room is finished loading, it will also **AUTOMATICALLY** call whatever function you define as `system.onload`. 

Therefore you can put any code in there that you want to run after the operating system is ready (which is all of it because you don't want to run any code until the operating system is ready).
...
**Now I can set the coffee cup's name attribute to the name the person gave me on the phone**

CRAP! I don't know how to refer to the person's name that I asked for or the coffee that I created!
...
There's already a take-out counter in the room so I don't need to create one. However, I don't have a reference to it yet so before I can even put the coffee on the take-out counter, I must 
...
**Almost! I want to run the code I wrote above whenever someone calls me on my take-out phone!**

Ok. So it seems like I need to program the take-out phone in this room as well and whenever it rings, I need it to run the code that I wrote above.
...
Therefore, I want to put the code I wrote above in the `phone.onring` function, like this:
...
Right now the ANY TIME the phone rings, it prepares a cup of coffee. We only want to do this when the operating system is ready or else the room could break.

This will solve the problem!

```js
//  system.ready function is AUTOMATCAILLY called by the room whenever it's operating system is ready. Therefore, there is NO code that runs until the operating system is ready.
...
Note that code that is commented in the below example is not executed at the given moment whereas code that is not commented is currently being executed.

1. When the code first runs: the only thing that happens is the definition of the `system.onload` function. None of the code in the `system.onload` function is executed yet.
...
2. Whenever the operating system has finished loading (it could be seconds, it could be hours), the code defined inside `system.onload` runs. Note that the code inside the `phone.onring` function however doesn't run yet.
...
3. Then whenever the phone rings (it could be minutes, it could be hours), the code inside the `phone.onring` function is run
